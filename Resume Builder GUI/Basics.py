...
class Basics:
    '''Class Basic Information : To collect your reference'''
    # '''-''' is called a doc-string->used to tell information

    def __init__(self, frame, mydata={}):
        # It it the constructor of class(frame is window,self reference is passed automatically)
    ...
    def start(self): # defining a function/method with class reference
        ...
        self.fname = Entry(frame1, fg="gray", bd=0, font=("Times New Roman", 12, "bold"), highlightbackground="Blue",
                           highlightthickness=2, textvariable=self.first_name)
        ...
        self.lname = Entry(frame1, fg="gray", bd=0, font=("Times New Roman", 12, "bold"),
                           highlightbackground="Blue", highlightthickness=2, textvariable=self.last_name)
        self.lname.place(relx=0.56, rely=0.08)      # x=400,y=50

        self.email = Entry(frame1, fg="gray", bd=0, font=("Times New Roman", 12, "bold"), highlightbackground="Blue",
                           highlightthickness=2, textvariable=self.Email)
        self.email.place(relx=0.12, rely=0.25)#x=200, y=100
        self.link = Entry(frame1, fg="gray", bd=0, font = ("Times New Roman", 12, "bold"), highlightbackground="Blue",
                          highlightthickness=2, textvariable=self.LinkedIn)
        self.link.place(relx= 0.56, rely=0.25)#x=400, y=100

        self.contact1 = Entry(frame1, fg="gray", bd=0, font = ("Times New Roman", 12, "bold"),
                              highlightbackground="Blue", highlightthickness=2, textvariable= self.cont1)
        self.contact1.place(relx=0.12, rely=0.42)#(x= 200, y=150)
        self.contact2 = Entry(frame1, fg="gray", bd=0, font = ("Times New Roman", 12, "bold"),
                              highlightbackground="Blue", highlightthickness=2, textvariable=self.cont2)
        self.contact2.place(relx=0.56, rely=0.42)#(x= 400, y=150)

        self.address1 = Entry(frame1, fg="gray", bd=0, font = ("Times New Roman", 12, "bold"),
                              highlightbackground="Blue", highlightthickness=2, width=50, textvariable=self.Address1)
        self.address1.place(relx=0.07, rely=0.6)#(x= 180, y=200)
        self.address2 = Entry(frame1, fg="gray", bd=0, font = ("Times New Roman", 12, "bold"),
                              highlightbackground="Blue", highlightthickness=2, width=50, textvariable=self.Address2)
        ...
        if self.data.__contains__('Info'):
            d = self.data['Info']
            self.first_name.set(d[0])
            self.last_name.set(d[1])
            self.Email.set(d[2])
            self.LinkedIn.set(d[3])
            self.cont1.set(d[4])
            self.cont2.set(d[5])
            self.Address1.set(d[6])
            self.Address2.set(d[7])
        else:
        ...
        next = Button(self.window, text="Next =>", font=("Times New Roman", 12, "bold"), bg="#30c2f2", command=self.Next
                      , relief=RIDGE, padx=5)
    ...
    def Next(self):#method
        ...
        # if self.data.__contains__('Info'):
        self.data['Info'] = (self.fname.get(), self.lname.get(), self.Email.get(), self.LinkedIn.get(),
            ...
            if self.Address2.get() != "-":
                ...
                for id in a_id_2:
                    if (id[1] == self.Address2.get()):
                        a_id_2 = id[0]
            # cur.execute(''INSERT INTO information VALUES(:a,:b,:c,:d,:e,:f,:g,:h)'',{"a":self.first_name.get(),
            "b":self.last_name.get(),"c":self.Email.get(),"d":self.LinkedIn.get(),"e":c_id_1,"f":c_id_2,"g":a_id_1,
            "h":a_id_2})
            connection.commit()
            connection.close()'''
...
if __name__ == "__main__":
    # Python representation of main function, actually it is a function that checks for cuurent file name
