...
class WorkExp:
    ...
    def start(self):
        ...
        '''else:
            self.job_title.set(self.data[2][0])
            self.company_name.set(self.data[2][1])
            self.start_year.set(self.data[2][2])
            self.end_year.set(self.data[2][3])
            self.others.insert(END,self.data[2][4])
        '''
        w = 150*2.2
        ...
        saveadd =Button(self.ui,text="Save & Add",font = ("Times New Roman",12,"bold"),relief=RIDGE,bg="#30f2f2",command=self.SaveAndAdd)
        saveadd.place(x= 425,rely= 0.75)
        done =Button(self.ui,text="Done",font = ("Times New Roman",12,"bold"),relief=RIDGE,bg="#30f2f2",command=self.Done)
    ...
    def submit(self):
        if(len(self.data) == 2):
            self.data.append((self.job_title.get(),self.company_name.get(),self.start_year.get(),self.end_year.get(),self.others.get(1.0,END)))
        else:
            self.data[2] = (self.job_title.get(),self.company_name.get(),self.start_year.get(),self.end_year.get(),self.others.get(1.0,END))
        print(self.data)
    def check_item(self):
    ...
    def SaveAndAdd(self):
        list1 = []
        list1.append((self.job_title.get(),self.company_name.get(),self.start_year.get(),self.end_year.get(),self.others.get(1.0,END)))
        print(list1)
        self.job_title.set("Job Title:")
        self.company_name.set("Company Name:")
        self.start_year.set("Start Year:")
    ...
    def Done(self):
        ...
        for i in range(len(self.life)):
            self.list_box.insert(END,self.life[0][i])
